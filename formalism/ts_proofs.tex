...
$x != y$ and that there exists $\Gamma_1, \Gamma_2, T_y, T_y'$ such that
...
$\typerule{\Gamma_1, x : V_2, y : T_y}{t''}{U}{\Gamma', x : T_3, y : V'}$.
Therefore, the rules of \tlet are satisfied such that
$\typerule{\Gamma, x : T'}{t}{U}{\Gamma', x : V'}$.
...
such that 
$\Gamma = \Gamma_1, y : \funt{U_i}{V_i}{U}, \overline{z_i : T_i}$
and that
$\Gamma' = \Gamma_1, y : \funt{U_i}{V_i}{U}, \overline{z_i : T_i'}$
...
\begin{itemize}
\item $x \neq y$ and $x \notin \overline{x_i}$, meaning that $x \in \Gamma_1$
and therefore $T = V$.
$\Gamma_1$ can be arbitrary in \tfunc, therefore we can apply
\tnarrow and choose $V' = T'$, giving 
$\typerule{\Gamma, x : T'}{y ( \overline{z_i} )}{U}{\Gamma', x : V'}$.

\item $x = y$. $T'$ must therefore be of form $\funt{U_i'}{V_i'}{U'}$ 
such that for each $i$, $U_i <: U_i'$, $V_i' <: V_i$, $U' <: U$. Additionally,
$V = T$. Let $V' = V$. It follows by rule \tfunc and \tsub that
$\typerule{\Gamma, x : T'}{x ( \overline{z_i})}{U}{\Gamma', x : V'}$.

\item $x \in \overline{z_i}$, meaning that $T = T_i$ and $V = T_i'$ for some $i$.
Let $V' = V$. The by rule \tnarrow,
$\typerule{\Gamma, x : T'}{y ( \overline{z_i})}{U}{\Gamma, x : V'}$.
...
\begin{itemize}
\item $x = y$. It follows by rule \tmethc that $T = O@\overline{S}$ for
some $O$ and $\overline{S}$, and that $V = O@\overline{S}'$ where
$S' = \{ S_n | m : T_r \Rightarrow S_n \in O@S, S \in \overline{S} \}$.
Additionally, 
$U = \bigsqcup{ \{ T_r | m : T_r \Rightarrow S_n \in O@S, S \in \overline{S} \} }$.

$T'$ must be of form $O'@\overline{S}''$. Let $V' = O'@\overline{S}'''$
where
$S''' = \{ S_n' | m : T_r' \Rightarrow S_n' \in O'@S'', S'' \in \overline{S}''\}$.
Let $U' = \bigsqcup{ \{ T_r' | m : T_r' \Rightarrow S_n' \in O'@S'', S'' \in \overline{S}'' \} }$.

By the definition of subtyping, 
$O'@\overline{S}'' \sim O@\overline{S}$, which implies that
$O'@\overline{S}''' \sim O@\overline{S}'$. Therefore, $U' <: U$
and $V' <: V$. By the rule \tsub, it follows that
$\typerule{\Gamma, x : T'}{x.m}{U}{\Gamma', x : V'}$.

\item $x \neq y$. It follows that $\Gamma = \Gamma'', y : O@\overline{S}$
and that $\Gamma' = \Gamma'', y : O@\overline{S'}$
for some $\Gamma''$, $O$ and $\overline{S}$.
As $x$ is irrelevant in the rule \tmethc in this case, it follows that
$T = V$. Let $V' = T'$. By the rule \tnarrow,
$\typerule{\Gamma', x : T'}{y.m}{U}{\Gamma, x : V'}$.
\end{itemize}
